...
static void recover_mstatus() {
  ...
  cpu.csr.mstatus = cpu.csr.mstatus | ((cpu.csr.mstatus >> 7) & 1) << 3;
  cpu.csr.mstatus = cpu.csr.mstatus | 0x80;
}
...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("0011000 00010 00000 000 00000 11100 11", mret, I, s->dnpc = cpu.csr.mepc);recover_mstatus();
  
  INSTPAT("0000000 ????? ????? 000 ????? 11100 11", ebreak, N, {
  uint32_t code = R(10) & 0x1;
  NEMUTRAP(s->pc, code);
}); // R(10) is $a0
